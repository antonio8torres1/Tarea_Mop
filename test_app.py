...
from streamlit import session_state, container,expander,selectbox,button,write,title,columns
...
with container():
    with expander("Configuracion del problema",expanded=True):
        session_state.config["target"] = selectbox("Escoja una opcion:",session_state.config["options"] )
        ...
        for i , col in enumerate(cols):
            with col:
                if i == session_state.config["num_variables"]:
                    st.selectbox("", session_state.config_table["opts_equations"], key=f"op{j}")
                elif i == session_state.config["num_variables"] + 1:
                    st.text_input("LD", key=f"ld{j}")
                else:
                    st.text_input(f"X{i + 1}", key=f"r{i}{j}")
